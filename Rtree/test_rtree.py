...
import tempfile
import shutil
from rtree_impl import RTreeIndex, Point, SpatialRecord, create_record
...
class TestSpatialRecord(unittest.TestCase):
    ...
    def test_record_serialization(self):
        ...
        self.assertEqual(
            restored_record.location.coordinates, record.location.coordinates
        )
...
def run_performance_test():
    ...
    for i in range(1000):
        record = create_record(
            i, [float(i % 100), float(i % 50)], {"data": f"Record {i}"}
        )
        rtree.add(record)
