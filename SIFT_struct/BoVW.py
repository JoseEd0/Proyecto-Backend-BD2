...
def crear_vocabulario_visual(
    all_desc, n_clusters=1000, random_state=42, output_path=None
):
    """Aplica MiniBatchKMeans a todos los descriptores para crear el vocabulario visual y lo guarda en disco si se indica"""
    print(
        f"Entrenando MiniBatchKMeans con {n_clusters} clusters sobre {all_desc.shape[0]} descriptores..."
    )
    kmeans = MiniBatchKMeans(
        n_clusters=n_clusters, random_state=random_state, batch_size=1000, n_init=3
    )
    kmeans.fit(all_desc)
    print("[OK] Vocabulario visual generado.")
    if output_path is not None:
        joblib.dump(kmeans, output_path)
        print(f"[OK] Diccionario visual guardado en {output_path}")
    return kmeans
...
def guardar_histogramas(histogramas, output_path):
    ...
    print(f"[OK] Histogramas guardados en {output_path}")
...
if __name__ == "__main__":
    ...
    crear_vocabulario_visual(
        all_desc, n_clusters=n_clusters, output_path=visual_dict_path
    )
