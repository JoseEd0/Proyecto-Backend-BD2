...
class MultimediaImageRetrieval:
    ...
    def __init__(
        ...
        self.descriptor_path = os.path.join(
            base_dir, "SIFT_struct/descriptors", "all_descriptors.npz"
        )
        self.visual_dict_path = os.path.join(
            base_dir, "SIFT_struct/descriptors", "visual_dictionary.pkl"
        )
        self.hist_path = os.path.join(
            base_dir, "SIFT_struct/descriptors", "all_histograms.npz"
        )
        self.tfidf_path = os.path.join(
            base_dir, "SIFT_struct/descriptors", "all_tfidf.npz"
        )
        self.RT = RegistroType(table_format, name_key=key)
        ...
        self.index_heap = Heap(
            dict_format, "pos", index_file_name, force_create=force_create
        )
    ...
    def _update_vocabulario(self):
        ...
        # Solo recalcular vocabulario después de tener al menos 10 imágenes
        with np.load(self.descriptor_path) as data:
            num_images = len(data.files)

        # No crear vocabulario hasta tener al menos 10 imágenes
        if num_images < 10:
            return

        # Primera vez: crear vocabulario con las primeras 10 imágenes
        if not os.path.exists(self.visual_dict_path):
            all_desc, _, _ = cargar_todos_los_descriptores(self.descriptor_path)
            crear_vocabulario_visual(
                all_desc, n_clusters=self.n_clusters, output_path=self.visual_dict_path
            )
        else:
            # Vocabulario incremental: recalcular cada 20 imágenes (menos frecuente)
            if num_images % 20 == 0:
                all_desc, _, _ = cargar_todos_los_descriptores(self.descriptor_path)
                crear_vocabulario_visual(
                    all_desc,
                    n_clusters=self.n_clusters,
                    output_path=self.visual_dict_path,
                )
    ...
    def insert(self, pos):
        ...
        4. Actualiza el vocabulario visual (visual_dictionary.pkl) si corresponde.
        5. Calcula y agrega el histograma BoVW a all_histograms.npz (solo si hay vocabulario).
        6. Actualiza el archivo all_tfidf.npz (solo si hay vocabulario).
        7. Inserta el registro (pos, nombre, vector tf-idf) en el heap del índice.

        Retorna: dict con información del estado de inserción
        """
        ...
        # 3. Actualizar vocabulario visual (solo cada 20 imágenes, o cuando alcancemos 10)
        self._update_vocabulario()

        # 4. Solo calcular histograma y TF-IDF si ya existe vocabulario
        has_vocabulary = os.path.exists(self.visual_dict_path)

        if has_vocabulary:
            # Calcular y agregar histograma BoVW
            data_dict = {nombre_base: descriptor}
            hist = construir_histogramas_por_imagen(data_dict, self.visual_dict_path)[
                nombre_base
            ]
            self._add_histograma_to_npz(nombre_base, hist)

            # Actualizar archivo tf-idf
            self._update_tfidf()

            # Insertar registro en Heap del índice con vector TF-IDF
            tfidf_vec = self._get_tfidf_vector(nombre_base)
            reg = [pos, nombre_base] + list(tfidf_vec)
            self.index_heap.insert(reg)
            print(
                f"[OK] Imagen '{nombre_base}' (pos {pos}) insertada en índice con TF-IDF."
            )
            return {"has_vocabulary": True, "status": "indexed"}
        else:
            # Sin vocabulario aún: solo insertar con vector dummy (se actualizará después)
            dummy_vec = np.zeros(self.n_clusters)
            reg = [pos, nombre_base] + list(dummy_vec)
            self.index_heap.insert(reg)
            ...
            print(
                f"[OK] Imagen '{nombre_base}' (pos {pos}) guardada. Vocabulario pendiente ({num_images}/10 imágenes)."
            )
            return {
    ...
    def knn_search(self, pos, k=3):
        ...
        if descriptor is None or len(descriptor) == 0:
            raise ValueError(
                "No se pudieron extraer descriptores de la imagen de consulta."
            )
        ...
        mats_base = np.array(
            [mats_tfidf[i] for i, k in enumerate(keys) if k != nombre_base]
        )
    ...
    def knn_search_with_similarity(self, image_path, k=10):
        """
        Busca las k imágenes más similares a partir de una ruta de imagen query.
        Esta imagen NO necesita estar en el índice.
        Retorna una lista de tuplas (posición, similitud) ordenadas por similitud descendente.
        """
        nombre_base = "__query_external__"
        ext = os.path.splitext(image_path)[1]
        img_tmp_path = os.path.join(self.img_dir, nombre_base + ext)

        # 1. Redimensionar y extraer descriptores de la imagen query
        resize_to_256_square(image_path, img_tmp_path, self.z)
        img = load_and_verify_image(img_tmp_path)
        sift = cv2.SIFT_create()
        _, descriptor = extract_descriptors(img, sift)
        if descriptor is None or len(descriptor) == 0:
            raise ValueError("No se pudieron extraer descriptores de la imagen query.")

        # 2. Cargar vocabulario visual y calcular histograma
        if not os.path.exists(self.visual_dict_path):
            raise ValueError(
                "No existe vocabulario visual. Debe indexar imágenes primero."
            )
        kmeans = joblib.load(self.visual_dict_path)
        words = kmeans.predict(descriptor)
        hist, _ = np.histogram(words, bins=np.arange(self.n_clusters + 1))

        # 3. Cargar todos los histogramas existentes y calcular TF-IDF
        if not os.path.exists(self.hist_path):
            raise ValueError("No existen histogramas. Debe indexar imágenes primero.")

        with np.load(self.hist_path) as data:
            histogramas = {key: data[key] for key in data.files}
        histogramas[nombre_base] = hist

        # Normalizar todos los histogramas
        for key in histogramas:
            h = histogramas[key]
            if len(h) < self.n_clusters:
                histogramas[key] = np.pad(h, (0, self.n_clusters - len(h)), "constant")
        ...
        # Alinear vectores tfidf con posiciones
        tfidf_base = []
        pos_base = []
        for nombre, posi in zip(nombre_list, pos_list):
            if nombre in keys_base:
                idx_base = keys_base.index(nombre)
                tfidf_base.append(mats_base[idx_base])
                pos_base.append(posi)
        tfidf_base = np.array(tfidf_base)
        ...
        for posi, sim in resultados:
            print(f"  pos {posi}: similitud = {sim:.4f}")
    ...
    def mostrar_heap(self, max_registros=10):
        print(f"Contenido de {self.index_heap.data_file}:")
        count = 0
        registros = self.index_heap._select_all()
        for reg in registros:
            # reg[1] es el nombre, puede ser string o bytes
            nombre = (
                reg[1]
                if isinstance(reg[1], str)
                else reg[1].decode("utf-8").rstrip("\x00")
            )
            pos = reg[0]
