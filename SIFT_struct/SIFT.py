...
def process_and_save_all_descriptors(
    images_dir, output_npz_path, n_components=None, batch_size=50
):
    """
    ...
    if n_components is not None and n_components < 128:
        print(
            f"Ajustando PCA para reducir descriptores a {n_components} dimensiones..."
        )
    ...
    print(f"\n[OK] Todos los descriptores guardados en {output_npz_path}")
...
def main():
    try:
        ...
        process_and_save_all_descriptors(
            test_images_dir, output_npz_path, n_components=None
        )
